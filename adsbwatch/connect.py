"""Native cognis-connect emit for adsbwatch — forward findings to any platform.

Maps adsbwatch's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    adsbwatch ... --format json | adsbwatch-emit --to stix
    adsbwatch-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = f"Anomaly in ADS-B record for {rec.get('callsign', 'unknown')}"
        out['severity'] = 'medium'
        out['type'] = 'adsb_anomaly'
        out['description'] = f"Detected anomaly in ADS-B record: {rec.get('kind', 'unknown')}"
        out['tags'] = ['adsb', 'anomaly']
